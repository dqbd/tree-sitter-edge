/**
 * Assumptions:
 * - Only support JavaScript tree-sitter
 * - Node.js should work seamlessly without resorting to calling Parser.init()
 * - We can afford to initialize the parser asynchronously in edge runtimes
 */
    ...
    static init(moduleOptions) {
      ...
      return (initPromise = new Promise((resolveInitPromise) => {
        ...
        function createWasm() {
          ...
          var ENVIRONMENT_IS_EDGE =
            typeof process == "object" &&
            typeof window != "object" &&
            typeof importScripts != "function"
          if (ENVIRONMENT_IS_EDGE) {
            import("./tree-sitter.wasm?module").then((module) =>
              WebAssembly.instantiate(module.default, info).then((instance) =>
                receiveInstance(instance, module.default)
              )
            )
            return {}
          }
        ...
        function getDylinkMetadata(binary) {
          ...
          if (
            ...
            binary.toString().includes("WebAssembly.Module")
          ) {
        ...
        class Language {
          ...
          static load(input) {
            let bytes
            if (input instanceof WebAssembly.Module || input.toString().includes("WebAssembly.Module")) {
        ...
        function unmarshalCaptures(query, tree, address, result) {
          for (let i = 0, n = result.length; i < n; i++) {
            ...
            address += SIZE_OF_NODE
            result[i] = { name: query.captureNames[captureIndex], node: node }
          }
          return address
        }
        ...
        function unmarshalNode(tree, address = TRANSFER_BUFFER) {
          ...
          result.startPosition = { row: row, column: column }
          result[0] = other
          return result
        }
